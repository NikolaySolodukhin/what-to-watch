  ...
  loadFavorite: () => (dispatch, _getState, api) => {
    if (_getState()[USER].userIsLoggedIn) {
      return api.get(`/favorite`)
        .then((response) => dispatch(ActionCreator.setFavorite(response.data)));
    }
    return false;
  },
  addFavorite: (filmId) => (dispatch, _getState, api) => api.post(`/favorite/${filmId}/1`)
      .then((response) => dispatch(ActionCreator.addFavorite(response.data))),

  removeFavorite: (filmId) => (dispatch, _getState, api) => api.post(`/favorite/${filmId}/0`)
      .then((response) => dispatch(ActionCreator.removeFavorite(response.data)))
};
